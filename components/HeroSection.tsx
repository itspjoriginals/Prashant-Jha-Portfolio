      ...
      <div className="container grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
          <motion.div initial="hidden" animate="show" variants={stagger} className="space-y-4">
            <motion.p variants={reveal} className="text-sm uppercase tracking-[0.4em] text-brand-accent">
              Frontend-Heavy Full Stack Developer
            </motion.p>
            <motion.h1 variants={reveal} className="mt-6 max-w-3xl text-5xl font-semibold leading-tight tracking-[-0.04em] text-black sm:text-6xl">
              I build fast, polished digital products with strong frontend craftsmanship.
            </motion.h1>
            <motion.p variants={reveal} className="mt-8 max-w-2xl text-base leading-7 text-black/80">
            ...
            <motion.div variants={reveal} className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <motion.div variants={reveal} className="mt-14 grid gap-4 sm:grid-cols-3">
              {proofHighlights.map((item) => (
                <motion.div key={item.label} variants={reveal} className="rounded-xl border border-brand-muted/50 bg-white p-5 shadow-soft">
